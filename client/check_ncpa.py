...
def parse_args():
    ...
    parser.add_option(
        "-H", "--hostname", help="The hostname to be connected to.")
    parser.add_option(
        "-M", "--metric", default='',
        ...
        "instruction.")
    parser.add_option(
        "-P", "--port", default=5693, type="int",
        help="Port to use to connect to the client.")
    parser.add_option(
        "-w", "--warning", default=None, type="str",
    ...
    parser.add_option(
        ...
        "as you would call from the command line. "
        "Example: -a '-w 10 -c 20 -f /usr/local'")
    parser.add_option(
        "-t", "--token", default=None,
        help="The token for connecting.")
    parser.add_option(
        "-T", "--timeout", default=15, type="int",
    ...
    if options.arguments and options.metric \
            and 'agent/plugin' not in options.metric:
        parser.print_help()
...
# I would normally split these out into
...
def get_check_arguments_from_options(options):
    ...
    to the end of the host URL. This is different from the
    get_arguments_from_options in that this is meant for the
    syntax when the user is calling a check, whereas the below
    ...
    if arguments is None:
        ...
        arguments = '/'.join(
            [urlquote(x, safe='') for x in shlex.split(arguments)])
        return arguments
...
def get_arguments_from_options(options, **kwargs):
    ...
    # Note that the distinction between units/unit is a bit confusing.
    # According to the API, the units the is the unit prefix,
    # like K, M or G. Unit is the unit that will be ascribed
...
def timeout_handler(threshold):
    def wrapped(signum, frames):
        stdout = "UNKNOWN: Execution exceeded timeout threshold of %ds" % \
            threshold
        print stdout
        sys.exit(3)
    return wrapped
...
def main():
    ...
    try:
        ...
        if options.super_verbose:
            ...
            return 'UNKNOWN: Error occurred while running the plugin.' \
                'Use the verbose flag for more details.', 3
