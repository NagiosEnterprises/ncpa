...
echo ""
echo "ex: # ./linux/build_ssl_and_python.sh"
echo ""
...
install_openssl() {
    # Skip if OpenSSL is already installed at the target location
    if [ -d "$INSTALL_DIR_OPENSSL" ]; then
        echo "OpenSSL already installed at $INSTALL_DIR_OPENSSL, skipping installation."
        return
    fi
...
# --- Compile and Install Python with Custom OpenSSL ---
install_python() {
    # Skip if Python is already installed at the target location
    if [ -d "$INSTALL_DIR_PYTHON" ]; then
        echo "Python already installed at $INSTALL_DIR_PYTHON, skipping installation."
        return
    fi
